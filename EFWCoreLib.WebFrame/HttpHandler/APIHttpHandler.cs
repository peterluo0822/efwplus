...
namespace EFWCoreLib.WebFrame.HttpHandler
{
    ...
    public class APIHttpHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                //if (AppGlobal.IsRun == false)
                //    throw new Exception("系统未正常启动！");
                ...
                else
                {
                    ...
                    context.Response.Write(RetError("controller or method is not empty!", null));
                }
            }
            catch (Exception err)
            {
                ...
                if (err.InnerException != null)
                    context.Response.Write(RetError(err.InnerException.Message, null));//执行异常
                else
                    context.Response.Write(RetError(err.Message, null));
                //记录错误日志
