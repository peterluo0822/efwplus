...
namespace EfwControls.Common
{
    ...
    public class HttpWebResponseUtility
    {
        ...
        public static object GetHttpData(string url, params string[] args)
        {
            foreach (string s in args)
            {
                url += "&" + s.Split(new char[] { ':' })[0] + "=" + s.Split(new char[] { ':' })[1];
            }
            HttpWebResponse response = HttpWebResponseUtility.CreateGetHttpResponse(url, null, null, null);
            //获取消息的Json数据
            object data = ToResult(HttpWebResponseUtility.GetHttpData(response));
            return data;
        }
        ...
        {
            if (json == "fail")
                throw new Exception("API命令验证失败");
            else if (json == "error")
                throw new Exception("API命令格式错误");
            else if (json == "exception")
                throw new Exception("API命令执行异常");
